// ============================================================
// UTIL: Costanti di gioco condivise tra client e server
// Porta TypeScript di src/lib/constants.js
// Raggruppate per dominio (rarità, aspetto, stat, timer, mappa).
// ============================================================

// ── TIPI ─────────────────────────────────────────────────────
export type RaritaKey = 'comune' | 'raro' | 'epico' | 'leggendario' | 'immersivo'

export interface RaritaConfig {
  nome:   string
  colore: string
  glow:   string
  stelle: number
...
export interface RarityMultiplierConfig {
  multiplier: number
  vel_min:    number
  vel_max:    number
  crit_min:   number
...
export interface MoveRangeConfig {
  pp_min:   number
  pp_max:   number
  danno_min:  number
  danno_max:  number
...
// ── VERSIONE STATS ────────────────────────────────────────────
/** Incrementare ad ogni migrazione delle stats waifu in Firestore. */
export const STATS_VERSION = 1

// ── MOLTIPLICATORI RARITÀ ─────────────────────────────────────
/** Fallback se Firestore non è disponibile; modificabili da Admin. */
export const RARITY_MULTIPLIERS_DEFAULT: Record<RaritaKey, RarityMultiplierConfig> = {
...
/** Range mosse attacco per rarità (fallback se Firestore non disponibile). */
export const MOVE_RANGES_DEFAULT: Record<RaritaKey, MoveRangeConfig> = {
...
/** Incrementi per level-up mosse attacco. */
export const MOVE_LEVELUP_DEFAULT = {
  incremento_danno:         5,
  incremento_danno_critico: 0, // legacy: danno_critico = round(danno × 1.25)
}

// ── RARITÀ ───────────────────────────────────────────────────
/**
 * Configurazione rarità waifu (gacha prob, colori, stelle).
 * La somma dei `prob` deve essere 1.00.
 */
export const RARITA: Record<RaritaKey, RaritaConfig> = {
...
// ── ASPETTO WAIFU ─────────────────────────────────────────────
export interface ColoreCapelliEntry {
  nome:  string
  hex:   string
  solid: string
}

/** Mappa ID colore capelli → dati rendering. `hex` può essere identificatore gradiente. */
export const COLORI_CAPELLI: Record<number, ColoreCapelliEntry> = {
...
/** Etichette categorie tette (stat range 1–7). */
export const CATEGORIE_TETTE: Record<number, string> = {
...
export const UPGRADE_STEPS_DEFAULT: Record<string, number> = {
  tette:          1,
  colore_capelli: 1,
  taglia_piedi:   1,
  eta:            25,
...
// ── UI — OUTFIT & STAT ────────────────────────────────────────
export const SLOT_OUTFIT: Record<string, { nome: string; icon: string }> = {
...
// ── TIMER & ENERGIA ──────────────────────────────────────────
/** Configurazione timer ricarica pacchetti ed energia. */
export const TIMER = {
  PACCHETTO_HOURS: 12,
  ENERGIA_HOURS:   12,
  MAX_ENERGIA:     10,
...
export const ENERGIA_SCARTO: Record<RaritaKey, number> = {
  comune: 1, raro: 2, epico: 3, leggendario: 5, immersivo: 8,
}

// ── MAPPA TERRITORI ──────────────────────────────────────────
/** 28 territori della mappa risiko-style (6 continenti). */
export const TERRITORI: TerritoryDef[] = [
...
/** Colori SVG per continente nella mappa risiko. */
export const COLORI_CONTINENTI: Record<string, string> = {
  NA: '#7c3aed', SA: '#fbbf24', EU: '#0ea5e9',
  AF: '#16a34a', AS: '#dc2626', OC: '#f97316',
}

/** Nomi leggibili dei continenti (per UI, legende, tooltip). */
export const NOMI_CONTINENTI: Record<string, string> = {
