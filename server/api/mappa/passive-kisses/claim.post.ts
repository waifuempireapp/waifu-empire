...
import { weightedTerritoryForUid } from '../../../utils/territoryScore';
...
export default defineEventHandler(async (event) => {
  try {
    ...
    // Punteggio PESATO sulla difficoltà: ogni territorio vale (difficoltà%/100)
    // → 1 al 100%, 0.27 al 27%. Premia i territori difficili, non la quantità.
    const weighted: number = await weightedTerritoryForUid(uid);
    if (weighted <= 0) return { earned: 0, message: 'Nessun pixel posseduto' };
    ...
    // Rate: punteggioPesato/4 * passiveRate per ora, floor SOLO sul totale finale.
    const earned: number = Math.floor((weighted / TERRITORIES_PER_KISS) * passiveRate * hoursElapsed);
    ...
    return { success: true, earned, weighted: Math.round(weighted * 100) / 100, hoursElapsed: Math.round(hoursElapsed * 10) / 10 };
  } catch (e: any) {
