// GET /api/cron/map-mission-snapshot
...
export async function GET(request) {
  ...
  try {
    ...
    for (const missionDoc of missionsSnap.docs) {
      ...
      const endsAt = mission.endsAt?.toMillis?.() ?? 0;
      ...
      if (endsAt >= windowStart.getTime() && endsAt <= windowEnd.getTime()) {
        ...
        for (const pixel of pixels) {
          const chunkId = pixel.chunkId || `chunk_${Math.floor(pixel.x / 10)}_${Math.floor(pixel.y / 10)}`;
