...
async function preloadAllImages(packList: typeof packs.value): Promise<void> {
  ...
  // TUTTO pre-renderizzato prima del pick: thumbnail del feed, immagini 'card'
  // usate nel reveal E il retro carta — niente caricamenti a vista durante flip/shuffle
  const urls = packList
    .flatMap(p => (p.cards ?? []).flatMap(c => c.immagine
      ? [iku(c.immagine, 'thumbnail'), iku(c.immagine, 'card')]
      : []))
    .filter((u): u is string => !!u)
  urls.push(backCardImg as unknown as string)
            ...
            <div
              ...
              <div style="display:flex; justify-content:center; gap:10px; transform-style:preserve-3d;">
                <div
                  ...
                  <template v-if="pickPhase === 'reveal'">
                    <img v-if="selectedPack.cards?.[0]?.immagine" :src="ikUrl(selectedPack.cards[0].immagine, 'card') ?? ''" style="width:100%;height:100%;object-fit:cover;object-position:center 15%;"/>
                  ...
                  <template v-else>
                    <div :style="{position:'absolute',inset:0,transformStyle:'preserve-3d',transition:'transform 0.55s cubic-bezier(0.4,0,0.2,1)',transform:(pickPhase==='revealing'||pickPhase==='revealed')&&inPlaceFlipped.has(0)?'rotateY(180deg)':'rotateY(0deg)'}">
                      ...
                      <div style="position:absolute;inset:0;backface-visibility:hidden;-webkit-backface-visibility:hidden;transform:rotateY(180deg);border-radius:13px;overflow:hidden;">
                        <video v-if="inPlaceCards[0]?.video" :src="inPlaceCards[0].video ?? undefined" autoplay muted loop playsinline @error="(e:any) => e.target.style.display='none'" style="width:100%;height:100%;object-fit:cover;object-position:center 15%;display:block;"/><img v-else-if="inPlaceCards[0]?.immagine" :src="ikUrl(inPlaceCards[0].immagine, 'card') ?? ''" style="width:100%;height:100%;object-fit:cover;object-position:center 15%;display:block;"/>
                        <div v-else style="width:100%;height:100%;display:grid;place-items:center;"><img src="~/assets/images/New_Logo.png" alt="" style="width:55%;opacity:0.75;"/></div>
                        ...
                        <PescaMoveOverlay :carta="inPlaceCards[0]" />
                      </div>
                    </div>
                  </template>
                </div>
                <div
                  ...
                  @click="onPickCard(1)">
                  <template v-if="pickPhase === 'reveal'">
                    <img v-if="selectedPack.cards?.[1]?.immagine" :src="ikUrl(selectedPack.cards[1].immagine, 'card') ?? ''" style="width:100%;height:100%;object-fit:cover;object-position:center 15%;"/>
                    <div v-else style="width:100%;height:100%;display:grid;place-items:center;"><img src="~/assets/images/New_Logo.png" alt="" style="width:55%;opacity:0.75;"/></div>
                  </template>
                  <!-- CSS 3D flip -->
                  <template v-else>
                    <div :style="{position:'absolute',inset:0,transformStyle:'preserve-3d',transition:'transform 0.55s cubic-bezier(0.4,0,0.2,1)',transform:(pickPhase==='revealing'||pickPhase==='revealed')&&inPlaceFlipped.has(1)?'rotateY(180deg)':'rotateY(0deg)'}">
                      ...
                      <div style="position:absolute;inset:0;backface-visibility:hidden;-webkit-backface-visibility:hidden;transform:rotateY(180deg);border-radius:13px;overflow:hidden;">
                        <video v-if="inPlaceCards[1]?.video" :src="inPlaceCards[1].video ?? undefined" autoplay muted loop playsinline @error="(e:any) => e.target.style.display='none'" style="width:100%;height:100%;object-fit:cover;object-position:center 15%;display:block;"/><img v-else-if="inPlaceCards[1]?.immagine" :src="ikUrl(inPlaceCards[1].immagine, 'card') ?? ''" style="width:100%;height:100%;object-fit:cover;object-position:center 15%;display:block;"/>
                        <div v-else style="width:100%;height:100%;display:grid;place-items:center;"><img src="~/assets/images/New_Logo.png" alt="" style="width:55%;opacity:0.75;"/></div>
                        ...
                        <PescaMoveOverlay :carta="inPlaceCards[1]" />
                      </div>
                    </div>
                  </template>
                </div>
              ...
              <div style="display:grid; grid-template-columns:repeat(3,1fr); gap:10px; transform-style:preserve-3d;">
                  <div :style="cardStyle(2)"
                    @click="onPickCard(2)">
                  <template v-if="pickPhase === 'reveal'">
                    <img v-if="selectedPack.cards?.[2]?.immagine" :src="ikUrl(selectedPack.cards[2].immagine, 'card') ?? ''" style="width:100%;height:100%;object-fit:cover;object-position:center 15%;"/>
                    <div v-else style="width:100%;height:100%;display:grid;place-items:center;"><img src="~/assets/images/New_Logo.png" alt="" style="width:55%;opacity:0.75;"/></div>
                  </template>
                  <!-- CSS 3D flip -->
                  <template v-else>
                    <div :style="{position:'absolute',inset:0,transformStyle:'preserve-3d',transition:'transform 0.55s cubic-bezier(0.4,0,0.2,1)',transform:(pickPhase==='revealing'||pickPhase==='revealed')&&inPlaceFlipped.has(2)?'rotateY(180deg)':'rotateY(0deg)'}">
                      ...
                      <div style="position:absolute;inset:0;backface-visibility:hidden;-webkit-backface-visibility:hidden;transform:rotateY(180deg);border-radius:13px;overflow:hidden;">
                        <video v-if="inPlaceCards[2]?.video" :src="inPlaceCards[2].video ?? undefined" autoplay muted loop playsinline @error="(e:any) => e.target.style.display='none'" style="width:100%;height:100%;object-fit:cover;object-position:center 15%;display:block;"/><img v-else-if="inPlaceCards[2]?.immagine" :src="ikUrl(inPlaceCards[2].immagine, 'card') ?? ''" style="width:100%;height:100%;object-fit:cover;object-position:center 15%;display:block;"/>
                        <div v-else style="width:100%;height:100%;display:grid;place-items:center;"><img src="~/assets/images/New_Logo.png" alt="" style="width:55%;opacity:0.75;"/></div>
                        ...
                        <PescaMoveOverlay :carta="inPlaceCards[2]" />
                      </div>
                    </div>
                  </template>
                  </div>
                  <div :style="cardStyle(3)"
                    @click="onPickCard(3)">
                  <template v-if="pickPhase === 'reveal'">
                    <img v-if="selectedPack.cards?.[3]?.immagine" :src="ikUrl(selectedPack.cards[3].immagine, 'card') ?? ''" style="width:100%;height:100%;object-fit:cover;object-position:center 15%;"/>
                    <div v-else style="width:100%;height:100%;display:grid;place-items:center;"><img src="~/assets/images/New_Logo.png" alt="" style="width:55%;opacity:0.75;"/></div>
                  </template>
                  <!-- CSS 3D flip -->
                  <template v-else>
                    <div :style="{position:'absolute',inset:0,transformStyle:'preserve-3d',transition:'transform 0.55s cubic-bezier(0.4,0,0.2,1)',transform:(pickPhase==='revealing'||pickPhase==='revealed')&&inPlaceFlipped.has(3)?'rotateY(180deg)':'rotateY(0deg)'}">
                      ...
                      <div style="position:absolute;inset:0;backface-visibility:hidden;-webkit-backface-visibility:hidden;transform:rotateY(180deg);border-radius:13px;overflow:hidden;">
                        <video v-if="inPlaceCards[3]?.video" :src="inPlaceCards[3].video ?? undefined" autoplay muted loop playsinline @error="(e:any) => e.target.style.display='none'" style="width:100%;height:100%;object-fit:cover;object-position:center 15%;display:block;"/><img v-else-if="inPlaceCards[3]?.immagine" :src="ikUrl(inPlaceCards[3].immagine, 'card') ?? ''" style="width:100%;height:100%;object-fit:cover;object-position:center 15%;display:block;"/>
                        <div v-else style="width:100%;height:100%;display:grid;place-items:center;"><img src="~/assets/images/New_Logo.png" alt="" style="width:55%;opacity:0.75;"/></div>
                        ...
                        <PescaMoveOverlay :carta="inPlaceCards[3]" />
                      </div>
                    </div>
                  </template>
                  </div>
                  <div :style="cardStyle(4)"
                    @click="onPickCard(4)">
                  <template v-if="pickPhase === 'reveal'">
                    <img v-if="selectedPack.cards?.[4]?.immagine" :src="ikUrl(selectedPack.cards[4].immagine, 'card') ?? ''" style="width:100%;height:100%;object-fit:cover;object-position:center 15%;"/>
                    <div v-else style="width:100%;height:100%;display:grid;place-items:center;"><img src="~/assets/images/New_Logo.png" alt="" style="width:55%;opacity:0.75;"/></div>
                  </template>
                  <!-- CSS 3D flip -->
                  <template v-else>
                    <div :style="{position:'absolute',inset:0,transformStyle:'preserve-3d',transition:'transform 0.55s cubic-bezier(0.4,0,0.2,1)',transform:(pickPhase==='revealing'||pickPhase==='revealed')&&inPlaceFlipped.has(4)?'rotateY(180deg)':'rotateY(0deg)'}">
                      ...
                      <div style="position:absolute;inset:0;backface-visibility:hidden;-webkit-backface-visibility:hidden;transform:rotateY(180deg);border-radius:13px;overflow:hidden;">
                        <video v-if="inPlaceCards[4]?.video" :src="inPlaceCards[4].video ?? undefined" autoplay muted loop playsinline @error="(e:any) => e.target.style.display='none'" style="width:100%;height:100%;object-fit:cover;object-position:center 15%;display:block;"/><img v-else-if="inPlaceCards[4]?.immagine" :src="ikUrl(inPlaceCards[4].immagine, 'card') ?? ''" style="width:100%;height:100%;object-fit:cover;object-position:center 15%;display:block;"/>
                        <div v-else style="width:100%;height:100%;display:grid;place-items:center;"><img src="~/assets/images/New_Logo.png" alt="" style="width:55%;opacity:0.75;"/></div>
                        ...
                        <PescaMoveOverlay :carta="inPlaceCards[4]" />
                      </div>
                    </div>
                  </template>
                  </div>
